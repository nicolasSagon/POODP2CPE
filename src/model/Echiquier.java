...
public class Echiquier {
	 ...
	 * <p>
	 * L'algo est le suivant : <br>
	 * s'il n'existe pas de piece du jeu courant aux coordonnées initiales -->
	 * false, <br>
	 * si les coordonnées finales ne sont pas valides ou égales aux initiales
	 * --> false, <br>
	 * si position finale ne correspond pas à algo de déplacement piece -->
	 * false, <br>
	 * s'il existe une piéce intermédiaire sur la trajectoire --> false (sauf
	 * cavalier), <br>
	 ...
	 * sinon : prendre la piéce intermédiaire (vigilance pour le cas du pion) et
	 * déplacer la piéce -->true,<br>
	...
	private boolean isMoveLegal(int xInit, int yInit, int xFinal, int yFinal) {
		...
		// s'il existe une piéce intermédiaire sur la trajectoire --> false
		// (sauf cavalier)
		...
		if (coordPieceInter != null) {
			...
			if (coordPieceInter.x == xFinal && coordPieceInter.y == yFinal) {
				...
				// si elle est de la méme couleur --> false ou potentiel
				// "roque du roi"
				if (this.jeuCourant.isPieceHere(xFinal, yFinal)) {
					this.jeuCourant.setCastling();
					isPieceOnPath = true; // à vérifier
				}
				// si elle n'est pas de la méme couleur, elle pourrait être
				// prise
				else {
					this.isCatchOk = true;
	...
	/**
	 * Permet de déplacer une piéce connaissant ses coordonnées initiales vers
	 * ses coordonnées finales. l'algo vérifie que le déplacement est légal,
	 * effectue ce déplacement avec l'éventuelle capture, rembobine si le
	 * déplacement et la capture ont mis le roi en échec
	 * 
	 * @param xInit
	 *            position initiale
	 * @param yInit
	...
	public boolean move(int xInit, int yInit, int xFinal, int yFinal) {
		...
		if (this.isMoveOk) {
			...
			this.setMessage("OK : déplacement sans capture "); // message sera
																// annulé si
																// capture
	...
	/**
	 * Permet de vérifier s'il existe une piece sur la trajectoire et retourne
	 * ses coordonnées si oui.
	 * 
	 * @param xInit
	 * @param yInit
	 * @param xFinal
	 * @param yFinal
	 * @return coordonnées de la 1ére piece intermédiaire rencontrée aprés scan
		...
		if (xInit == xFinal) {
			...
			if (yFinal > yInit) {
				for (int i = yInit + 1; i <= yFinal; i++) {
					if (jeuBlanc.isPieceHere(xInit, i)
							|| jeuNoir.isPieceHere(xInit, i)) {
			...
			if (yInit == yFinal) {
				...
				if (xFinal > xInit) {
					for (int i = xInit + 1; i <= xFinal; i++) {
						if (jeuBlanc.isPieceHere(i, yInit)
								|| jeuNoir.isPieceHere(i, yInit)) {
				...
				if (Math.abs(yInit - yFinal) == Math.abs(xInit - xFinal)) {
					...
					if ((xFinal - xInit > 0) && (yFinal - yInit > 0)) {
						for (int i = 1; i <= ecart; i++) {
							if (jeuBlanc.isPieceHere(xInit + i, yInit + i)
									|| jeuNoir
											.isPieceHere(xInit + i, yInit + i)) {
								pieceOnTrajectCoord = new Coord(xInit + i,
										yInit + i);
								break;
							}
					...
					if (jeuBlanc.isPieceHere(xFinal, yFinal)
							|| jeuNoir.isPieceHere(xFinal, yFinal)) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see java.lang.Object#toString() pour retourner le plateau de jeu sous la
	 * forme suivante 0 1 2 3 4 5 6 7 0 N_T1 N_C1 N_F1 N_r_ N_R_ N_F2 N_C2 N_T2
	 * 1 N_P1 N_P2 N_P3 N_P4 N_P5 N_P6 N_P7 N_P8 2 __ __ __ __ __ __ __ __ 3 __
	 * __ __ __ __ __ __ __ 4 __ __ __ __ __ __ __ __ 5 __ __ __ __ __ __ __ __
	 * 6 B_P1 B_P2 B_P3 B_P4 B_P5 B_P6 B_P7 B_P8 7 B_T1 B_C1 B_F1 B_r_ B_R_ B_F2
	 * B_C2 B_T2
	 */
	...
	public String toString() {
		...
		for (int i = 0; i < 8; i++) {
			...
			for (int j = 0; j < 8; j++) {
				...
				String nomPiece = (nomPieceBlanc != null ? nomPieceBlanc
						: nomPieceNoir);
